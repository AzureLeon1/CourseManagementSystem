<template>
  <div class="send-twitter">
    <el-input type="textarea" :autosize="{ minRows: 3, maxRows: 5 }" placeholder="请输入内容" v-model="textarea" maxlength="140" resize="none" :class="{withImg: dialogImageUrl}">
    </el-input>

    <img class="preview" :src="dialogImageUrl" alt="图片加载中" v-if="dialogImageUrl">
    <div class="bottom">

      <div class="word-count">
        <span>{{wordCount}} / 140</span>
      ...
      <div class="btns">
        ...
        >
          <el-button size="small">{{ dialogImageUrl ? '替换' : '添加' }}图片</el-button>
        </el-upload>
        <el-button type="primary" size="small" @click="emit">发 布</el-button>
      </div>
    </div>
  </div>
</template>
